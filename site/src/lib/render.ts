...
function samplePoints(n: number, l: number, m: number, count: number) {
	...
	while (points.length < count * 3) {
		...
		const phi = 2 * o.PI * Math.random();

		const p = o.probabilityDensity(n, l, m, r, theta, phi);
...
let data = samplePoints(4, 2, 0, 6_500);
...
const proj = perspective(o.PI / 4, canvas.width / canvas.height, 0.1, 100);
gl.uniformMatrix4fv(uProj, false, proj);

let angle = 0.1;

export function generatePoints() {
	const data = samplePoints(1, 0, 0, 6_500);
