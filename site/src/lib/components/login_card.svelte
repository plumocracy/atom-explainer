<script lang="ts">
	...
	import { signIn, type Provider } from '$lib/auth-client';

	let { loginPrompt } = $props() 
...
<div class="flex h-screen w-full items-center justify-center text-zinc-300">
	<div class="flex h-1/4 w-1/3 flex-col items-center rounded-2xl">
		<div class="absolute mt-4 text-4xl text-center">
			{#if !loginPrompt}
		...
		<div class="my-auto flex flex-row space-x-10 text-4xl">
			{#each LOGIN_PROVIDERS as provider}
				<button aria-label={provider.ariaLabel} onclick={() => handleSignIn(provider.providerName)}>
