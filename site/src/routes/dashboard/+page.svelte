<script lang="ts">
	...
	const activeConversation = $derived(
		data.selectedConversationId
			? data.conversations.find((conversation) => conversation.id === data.selectedConversationId) ?? null
			: null
	);
	...
	<section class="museum-frame p-4 md:p-6 lg:p-8">
		<header class="museum-panel rounded-3xl border px-5 py-5 shadow-[var(--museum-shadow)] md:px-7 md:py-6">
			<div class="flex flex-wrap items-center justify-between gap-4">
				...
				<a href="/" class="museum-button rounded-full px-4 py-2 text-sm font-semibold">Back to exhibit</a>
			</div>
		</header>
		...
		<div class="mt-5 grid min-h-0 flex-1 gap-5 lg:grid-cols-[340px_minmax(0,1fr)]">
				...
				<section class="museum-panel rounded-2xl border p-5">
					...
					<div class="mt-4 grid gap-3 text-sm">
						<div class="rounded-xl border border-[var(--museum-stroke)] bg-white/40 px-4 py-3">
							<p class="museum-kicker !tracking-[0.12em]">Input Tokens</p>
					...
					{:else}
						<ul class="mt-4 space-y-2">
							{#each data.conversations as conversation}
								<li>
									<a
										href={`/dashbord?conversation=${conversation.id}`}
										class="block rounded-xl border px-3 py-3 text-sm transition {conversation.id === data.selectedConversationId
			...
			<section class="museum-panel min-h-[320px] min-w-0 overflow-hidden rounded-2xl border p-5 md:p-6">
				{#if !activeConversation}
					<p class="text-sm text-[var(--museum-subtext)]">
						Select a conversation to view its message history.
					</p>
				{:else}
					<div class="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--museum-stroke)] pb-4">
						<div>
						...
						<div class="text-sm text-[var(--museum-subtext)]">
							<p>Input: <span class="font-semibold tabular-nums">{activeConversation.promptTokens}</span></p>
							<p>Output: <span class="font-semibold tabular-nums">{activeConversation.completionTokens}</span></p>
						</div>
					</div>

					{#if data.history.length === 0}
						<p class="mt-6 text-sm text-[var(--museum-subtext)]">No messages recorded for this conversation.</p>
					{:else}
						<div class="mt-6 max-h-[60dvh] overflow-y-auto pr-1">
							<ul class="space-y-3">
								{#each data.history as message}
									<li
										class="rounded-xl border px-4 py-3 text-sm leading-relaxed {message.role === 'user'
											? 'border-[rgba(39,80,86,0.3)] bg-[rgba(39,80,86,0.08)]'
											: 'border-[rgba(184,138,71,0.35)] bg-[rgba(184,138,71,0.1)]'}"
									>
										<div class="mb-2 flex items-center justify-between gap-3 text-xs uppercase tracking-[0.14em] text-[var(--museum-subtext)]">
